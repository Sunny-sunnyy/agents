...
# Path configuration for Week 6 MCP
...
async def scrape_lesson_transcript(page, lesson_num, filepath):
    ...
    if not target_item:
        print(f"Error: Could not find lesson item {lesson_num} in the sidebar. Please make sure the section is expanded.")
        return False
    ...
    print(f"Navigating to lesson {lesson_num}. Waiting 3.5 seconds for page load...")
...
async def main():
    ...
    if not file_map:
        print("No lesson files found in G:\\Agent2026Win\\agents\\6_mcp\\tai_lieu.")
        return
        
    # We want to process lessons 109 to 131
    lessons_to_process = [l for l in range(109, 132) if l in file_map]
