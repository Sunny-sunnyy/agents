...
# Path configuration
TAI_LIEU_DIR = r"G:\Agent2026Win\agents\4_langgraph\tai_lieu"

def get_file_map():
    """
    Scans the target directory and maps lesson numbers to file paths.
    E.g., 69 -> 'G:\\...\\69. Day 1 - ... .txt'
    """
    file_map = {}
...
async def scrape_lesson_transcript(page, lesson_num, filepath):
    ...
    print(f"Navigating to lesson {lesson_num}. Waiting 3.0 seconds for page load...")
    ...
    if toggle_btn:
        await toggle_btn.click(force=True)
        print("Clicked transcript toggle to open panel. Waiting 2.0 seconds...")
    ...
    if not cues:
        # Try a more generic selector for the sidebar content
        cues = await page.query_selector_all('[class*="transcript--cue-container"] p, [class*="transcript--cue-container"] span')
...
async def main():
    ...
    # Process lessons 69 to 91
    lessons_to_process = [l for l in range(69, 92) if l in file_map]
